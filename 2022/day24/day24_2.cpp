...
bool operator%(const c3Point& lhs, const c3Point& rhs){
...
class cSolve{
	...
	void solve(char mark, bool fwd = true){

		queue<c3Point> que;
		int zOld = -1;
		// string outfile = name + ".output";
		// ofstream ofstr(outfile, ios::binary);
		c3Point startPoint(steps % vol.depth, 0, 1);
		c3Point finishPoint(steps % vol.depth, vol.height - 1, vol.width - 2);
		if(!fwd){
			swap(startPoint, finishPoint);
		}
