...
public:
	...
	bool IsValid(const string_view& str, vector<string_view>& stack, Trie* parent = nullptr) {
		if (parent == nullptr) {
			parent = this;
		...
		if (next->terminal) {
			if (str.size() == 1) {
				assert(next->pattern);
				stack.insert(stack.begin(), *next->pattern);
				return true;
			}
			if (parent->IsValid(str.substr(1), stack, parent)) {
				assert(next->pattern);
				stack.insert(stack.begin(), *next->pattern);
				return true;
			}
		...
		return str.size() != 1 &&
			next->IsValid(str.substr(1), stack, parent); 
	}
...
void Solve() {
	...
	for (const string& design : designs) {
		...
		bool a = tr.IsValid(design, solution);
		success += a;
		...
		if (a) {
			cout << "[";
			bool flag = false;
			for (const string_view& towel : solution) {
				if(flag) {
					cout << ", ";
				}
				flag = true;
