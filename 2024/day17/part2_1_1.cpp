...
set<ull> SolveCycle(const set<ull>& end_A, int i) {
	set<ull> start_A;
	for (ull end_a : end_A) {
		for(ull a = 0; a < 8; a++) {
			Reset();
			...
			if (program[i] == output.back() ) { 
				start_A.insert(new_a);
			}
		}
	}
...
void Solve() {
	...
	PrintState();
	set<ull> variants = {0};
	cout << "cycle " << program.size() << ", vars " << variants.size() << endl;
	for (int i = program.size() -1; i >= 0; i--) {
		variants = SolveCycle(variants, i);
		if (variants.empty()) {
			cout << "simulation failed in cycle " << i << endl;
			break;
		}
		cout << "cycle " << i << ", vars " << variants.size() << endl;
		cout << variants << endl;
	}
