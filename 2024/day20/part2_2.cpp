...
class BoardWalker : public Walker<Point> {
	const Board& graph_;
public:
	BoardWalker(const Board &board) 
		: graph_(board){
	}
protected:
...
int main(int argc, char** argv) {
	...
	for (int i = 0; Point A :path) {
		Point ofs;
		for (ofs.y = -CHEAT_LENGTH; ofs.y <= CHEAT_LENGTH; ofs.y++) {
			for (ofs.x = -CHEAT_LENGTH; ofs.x <= CHEAT_LENGTH; ofs.x++) {
				if (Norm1(ofs) > CHEAT_LENGTH) {
					continue;
				}
				Point B = A + ofs;
				if (board.OutOf(B)
				...
				long long benefit = image[B.y][B.x] - image[A.y][A.x] - Norm1(ofs);
				if (benefit <= 0) {
					continue;
				}
