...
struct GraphMover {
	...
	void Add(const T& prev, const T& node, size_t cost) {
		...
		else if (costs[node] == cost) {
			froms[node].insert(prev);
		}
	...
	void PrintAny(const Position& p, int i = 0) {
		//cout << "any " << i << ", count:" << froms.count(p) << endl;
		if (froms.count(p) == 0) {
			return;
		}
		for (const Position& x : froms.at(p)) {
			if (Get(x.pos) != 'O') {
				any_counter++;
				Set(x.pos, 'O'); 
			}
