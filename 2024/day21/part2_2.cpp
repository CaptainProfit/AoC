...
string IteratePath(const string& path) {
	string result = "A";
	for (long i = 1; i < path.length(); i++) {
		...
		result += panel_mapper_unique.at(key);
	}
	return result;
}
...
int main(int argc, char** argv) {
	...
	Init();	
	//Print(panel_mapper);
	string keyorder = "AV"s;
	for (int iter = 0; iter < 4; iter++) {
		string new_keyorder;
		new_keyorder = IteratePath(keyorder);
		keyorder = move(new_keyorder);
		cout << "iter : " << iter + 1 << endl;
		
		cout << keyorder.length() - 1;
		cout << endl;
		cout << "------"s << endl;
		/*
		iter 1: {3, 2}
