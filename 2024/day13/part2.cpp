...
void ReadFile(string filename) {
	...
	games.back().prize.x = stol(line3.c_str() + line3.find('X') + 2) + OFS;
	games.back().prize.y = stol(line3.c_str() + line3.find('Y') + 2) + OFS;
	getline(f, line1);
    }
}
...
ull Solve1(const Game& game) {
	...
	bool correct = (a11*x1 + a12*x2 == b1 && a21*x1 + a22*x2 == b2);
	cout << "(" << x1 << ", " << x2 << ")" << correct << (x1 < 0) << (x2 < 0) << endl;
	if (!correct || x1 < 0 || x2 < 0) {
		return 0;
	}
	return 3*x1 + x2;
}
